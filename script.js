// Your web app's Firebase configuration
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function renderFixedCosts() {
            ...
            (currentData.fixedCosts || []).forEach((cost, index) => {
                ...
                li.innerHTML = `
                    ...
                    ${payerHtml}
                    <button class="btn btn-danger" data-index="${index}" style="flex: 0.5;">削除</button>
                `;
        ...
        addFixedCostBtn.addEventListener('click', () => {
            const lastCost = currentData.fixedCosts[currentData.fixedCosts.length - 1];
            if (lastCost && !lastCost.name.trim() && !lastCost.amount) {
                alert('最後の固定費項目が空です。先に内容を入力してください。');
                return;
            }
